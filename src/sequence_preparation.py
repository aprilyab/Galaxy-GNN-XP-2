...
def calculate_optimal_sequence_length(sequences: List[List[str]], percentile: int = 95) -> int:
    """Calculate optimal sequence length based on data distribution."""
...
def augment_sequences(sequences: List[List[str]], augment_factor: float = 1.5) -> List[List[str]]:
    """Apply data augmentation to increase training data size."""
    ...
    for seq in sequences:
        ...
        # Generate subsequences (sliding window)
        for length in range(3, len(seq)):
            if np.random.random() < 0.3:  # 30% chance to include each subsequence
...
def save_variable_length_csv(seqs: List[List[str]], vocab: Vocabulary, output_path: Path, name: str):
    """Save sequences with variable lengths to CSV format."""
    if not seqs:
...
def main():
    ...
    # 1. Generate enhanced sequences from Connections TSV
    ...
    # 3. Apply data augmentation to training set only
    ...
    # 4. Build Vocabulary strictly from TRAINING split
    vocab = Vocabulary()
    vocab.build_from_sequences(train_seqs_augmented, min_count=3)  # Lower min_count for more coverage
    vocab.save(OUT_DIR / "vocab.json")
    logger.info(f"Vocabulary build complete. Size: {len(vocab.stoi)}")

    # 5. Build Transition Graph strictly from TRAINING split for negative sampling
    ...
    # 6. Calculate optimal sequence length
    ...
    # 7. Process each split and save CSVs with variable-length support
