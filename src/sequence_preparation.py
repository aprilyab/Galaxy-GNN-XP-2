...
from src.utils import setup_logger, save_json, Vocabulary, SequenceDataset, split_workflows, generate_negative_samples
...
def main():
    ...
    for name, seqs in [("train", train_s), ("val", val_s), ("test", test_s)]:
        ...
        ds = SequenceDataset(seqs, vocab)
        if ds.X:
            torch.save(torch.stack(ds.X), OUT / f"X_{name}.pt")
            torch.save(torch.stack(ds.y), OUT / f"y_{name}.pt")
            torch.save(generate_negative_samples(vocab, len(seqs)), OUT / f"{name}_neg.pt")
